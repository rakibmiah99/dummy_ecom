      ...
      <nav aria-label="Breadcrumb">
        <ol role="list" class="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
          <li v-for="breadcrumb in product.breadcrumbs" :key="breadcrumb.id">
            <NuxtLink :to="breadcrumb.href" class="flex items-center">
              <span class="mr-2 text-sm font-medium uppercase text-gray-900">{{ breadcrumb.name }}</span>
...
<!--                <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating" :class="[reviews.average > rating ? 'text-gray-900' : 'text-gray-200', 'h-5 w-5 flex-shrink-0']" aria-hidden="true" />-->
              ...
              <a :href="reviews.href" class="ml-3 text-sm font-medium text-indigo-600 hover:text-indigo-500">{{ reviews.available }} available quantity</a>
            </div>
          </div>
...
try {
  var product_details = reactive(await $fetch(url, {
    method: 'GET'
  }))
}
catch (error){
...
const product = {
  ...
  href: '#',
  breadcrumbs: [
    { id: 1, name: product_details.category, href: '/category/'+product_details.category },
    { id: 2, name: product_details.brand, href: '#' },
  ],
  images: [
    {
      src: product_details?.thumbnail ?? 'https://tailwindui.com/img/ecommerce-images/product-page-02-secondary-product-shot.jpg',
      alt: 'Two each of gray, white, and black shirts laying flat.',
    },
    {
      src: product_details?.images[1] ??'https://tailwindui.com/img/ecommerce-images/product-page-02-tertiary-product-shot-01.jpg',
      alt: 'Model wearing plain black basic tee.',
    },
    {
      src: product_details?.images[2] ?? 'https://tailwindui.com/img/ecommerce-images/product-page-02-tertiary-product-shot-02.jpg',
      alt: 'Model wearing plain gray basic tee.',
    },
    {
      src: product_details?.images[3] ?? 'https://tailwindui.com/img/ecommerce-images/product-page-02-featured-product-shot.jpg',
...
const reviews = { href: '#', average: product_details.rating, totalCount: 117 , available: product_details.stock}
